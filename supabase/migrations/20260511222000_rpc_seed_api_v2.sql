-- Recria a função RPC com Security Definer para bypass de RLS
...
DECLARE
    u RECORD;
BEGIN
    -- 1. Inserir Content (sem duplicar)
    ...
    -- 2. Criar a Atividade para cada utilizador registado
    FOR u IN (SELECT id FROM auth.users) LOOP
        IF NOT EXISTS (
            SELECT 1 FROM public.activities WHERE user_id = u.id AND content_id = p_content_id
        ) THEN
            INSERT INTO public.activities (
                title, description, category, type, user_id, content_id, focus_mode_enabled, shortcuts
            ) VALUES (
                p_title, p_description, p_activity_category, p_activity_type,
                u.id, p_content_id, false, false
            );
        END IF;
    END LOOP;
END;
