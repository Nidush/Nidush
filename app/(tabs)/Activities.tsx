...
const UnifiedActivitiesScreen = () => {
  ...
  const loadTemplates = useCallback(async () => {
    try {
      const [catalogActivities, userScenarios] = await Promise.all([
        fetchActivityTemplates().catch(() => []),
        fetchUserScenarios().catch(() => []),
      ]);
      setActivityTemplates(catalogActivities);
  ...
  const processedData = useMemo(() => {
    const dedupeById = <T extends Activity | Scenario>(items: T[]): T[] => {
    ...
    const catalogData =
      viewMode === 'activities'
        ? dedupeById(activityTemplates)
        : dedupeById(scenarioTemplates);
    ...
    const myCreationsList =
      viewMode === 'activities'
        ? filteredUserActivities.filter((item) => item.category !== 'AI For You')
        : filteredCatalog.filter((item) => item.category === 'My creations');

    const autoGeneratedRecommendations =
      viewMode === 'activities'
  ...
  const recommendedData = useMemo(() => {
    const persistedAiRecommendations =
      viewMode === 'activities'
        ? processedData.autoGeneratedRecommendations.map((item) => ({
            ...item,
    ...
    const liveAiRecommendations =
      viewMode === 'activities' && aiRecommendedIdeas.length > 0
        ? aiRecommendedIdeas.map((idea) => ({
            id: idea.id,
            title: idea.title,
    ...
    const fallbackRecommendations = processedData.recommended.slice(0, 5).map((item) => ({
      ...item,
    ...
    return [...persistedAiRecommendations, ...liveAiRecommendations, ...fallbackRecommendations]
      .filter((item) => {
        const key = String(item.id);
        if (seen.has(key)) return false;
        seen.add(key);
    ...
    aiRecommendedIdeas,
    isSavingAiRecommendationId,
    processedData.autoGeneratedRecommendations,
    processedData.recommended,
